...
class HTMLArticleForm(ArticleForm):
    class Meta(ArticleForm.Meta):
        exclude = ArticleForm.Meta.exclude + ('markup',)

    def save(self):
        self.instance.markup = 'html'
        return super(HTMLArticleForm, self).save()


class SearchForm(forms.Form):
    q = forms.CharField(required=True, label='Search')
    title_only = forms.BooleanField(required=False, label='Title only?')
